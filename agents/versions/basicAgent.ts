...
import { addMessage, callLLM, extractQueryFromMarkdown, getTableDefinitions, limitQuery, queryDB } from "../helpers"
...
export default async function askAgent(question: string, db: Database, onMessageCallback: Function, model?: string) {
	...
	let query;
	try {
		const llmQuery = await callLLM([systemMessage, userMessage], onMessageCallback, model);
		query = extractQueryFromMarkdown(llmQuery);
